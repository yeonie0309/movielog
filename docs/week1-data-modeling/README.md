...
- 생성된 ERD: https://www.erdcloud.com/d/N2Q539qkpjMmdgPpy

`schema.sql`을 MySQL DDL로 가져와 12개 테이블을 생성하고 관계 중심 자동 배치를
적용했다. 엔티티 목록에서 12개 테이블을 확인하고 최종 상태의 스냅샷도 생성했다.
제출할 때는 전체 테이블명·컬럼명·타입·관계선이 보이도록 PNG를 다운로드하거나
화면을 캡처한다.
...
- [x] ERDCloud 새 프로젝트를 생성하고 12개 테이블·관계 및 스냅샷을 확인했다.
- [ ] ERDCloud 전체 PNG 또는 화면 캡처를 원본 노션 미션 페이지에 첨부한다.
